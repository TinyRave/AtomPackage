...
log = function(message) {
  return postMessage(["log", message]);
};
...
self.addEventListener('message', function(message) {
  ...
  if (message.data[0] === "generate") {
    ...
    timeOffset = samplesGenerated / sampleRate;
    ...
    if (typeof buildSample !== "undefined" && buildSample !== null) {
      for (i = j = 0, ref1 = channelBufferSize; 0 <= ref1 ? j < ref1 : j > ref1; i = 0 <= ref1 ? ++j : --j) {
        sample = buildSample(samplesGenerated / sampleRate);
        samplesGenerated++;
      ...
      return log("You must define a buildSample() function!");
    }
  }
});
"""
